...
void NODE_INFO::setStatus(){
	isInRing = true;
}

void NODE_INFO::setSuccessor(string ip,int port,lli hash){
	successor.first.first = ip;
	successor.first.second = port;
	successor.second = hash;
}

void NODE_INFO::setSuccessorList(string ip,int port,lli hash){
	for(int i=1;i<=R;i++){
		successorList[i] = make_pair(make_pair(ip,port),hash);
	}
}

void NODE_INFO::setPredecessor(string ip,int port,lli hash){
	predecessor.first.first = ip;
	predecessor.first.second = port;
	predecessor.second = hash;	
}

void NODE_INFO::setId(lli nodeId){
	id = nodeId;
}

void NODE_INFO::setFingerTable(string ip,int port,lli hash){
	for(int i=1;i<=M;i++){
		fingerTable[i] = make_pair(make_pair(ip,port),hash);
	}
}

void NODE_INFO::storeKey(lli key,string val){
	dictionary[key] = val;
}

void NODE_INFO::printKeys(){
...
void NODE_INFO::updateSuccessorList(){
...
vector< pair<lli , string> > NODE_INFO::getAllKeysForSuccessor(){
	map<lli,string>::iterator it;
...
vector< pair<lli , string> > NODE_INFO::getKeysForPredecessor(lli nodeId){
	map<lli,string>::iterator it;
...
void NODE_INFO::stabilize(){

	/* get predecessor of successor */

	HelperFunctions help;

	string ownIp = sp.GET_IP();
	int ownPort = sp.GET_PORT();
...
void NODE_INFO::checkPredecessor(){
	...
	if(help.isNodeAlive(ip,port) == false){
		...
		if(predecessor.second == successor.second){
			successor.first.first = sp.GET_IP();
			successor.first.second = sp.GET_PORT();
			successor.second = id;
...
string NODE_INFO::getValue(lli key){
